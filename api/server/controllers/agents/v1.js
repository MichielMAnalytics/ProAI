...
const injectSpecialVariables = (instructions = '') => {
  ...
  const hasConnectionSection =
    instructions.includes('--- App Connection Instructions ---') &&
    instructions.includes('--- End App Connection Instructions ---');
  ...
  */
  ...
  if (!hasConnectionSection) {
    ...
    logger.info(
      `[injectSpecialVariables] App connection instructions section already exists, skipping injection`,
    );
  }
  return result;
};
