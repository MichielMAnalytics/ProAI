...
const buildOptions = async (req, endpoint, parsedBody, endpointType) => {
  ...
  if (isEphemeral && req.app.locals.addUserSpecificMcpFromDb && req.user?.id) {
    logger.debug(
      `[agents/buildOptions] Pre-loading MCP tools for ephemeral agent (user: ${req.user.id})`,
    );
    ...
    try {
      ...
      if (mcpResult.success) {
        logger.info(
          `[agents/buildOptions] MCP pre-loading successful: ${mcpResult.serverCount} servers, ${mcpResult.toolCount} tools in ${mcpResult.duration}ms`,
        );
      } else {
