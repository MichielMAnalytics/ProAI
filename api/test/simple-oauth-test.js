...
console.log('DEBUG: PIPEDREAM_CLIENT_SECRET =', process.env.PIPEDREAM_CLIENT_SECRET ? 'SET' : 'MISSING');
...
async function testDirectTokenRequest() {
  ...
  try {
    ...
    if (response.data && response.data.access_token) {
      ...
      console.log(`🕐 Expires at: ${new Date(Date.now() + response.data.expires_in * 1000).toISOString()}`);
...
function testAuthenticationErrorDetection() {
  ...
  testErrorMessages.forEach((message, index) => {
    ...
    if (isAuthError === shouldBeAuth) {
      console.log(`✅ Error ${index + 1}: Correctly ${isAuthError ? 'detected' : 'ignored'} as auth error`);
      passedTests++;
    } else {
      console.log(`❌ Error ${index + 1}: Incorrectly ${isAuthError ? 'detected' : 'ignored'} as auth error`);
