...
describe('Pipedream Connect End-to-End Integration', () => {
  ...
  beforeAll(async () => {
    ...
    console.log('PIPEDREAM_API_BASE_URL:', process.env.PIPEDREAM_API_BASE_URL || 'Not set (will use default)');
    ...
    try {
      ...
      console.log('Available methods:', Object.getOwnPropertyNames(testClient).filter(name => typeof testClient[name] === 'function'));
    } catch (error) {
  ...
  describe('Step 1: Generate Connect Token (Backend)', () => {
    test('should create a connect token using PipedreamConnect service', async () => {
    ...
    test('should create connect token directly using SDK client', async () => {
  ...
  describe('Step 2: Simulate Frontend Account Connection', () => {
    ...
    test('should simulate successful account connection', async () => {
  ...
  describe('Step 3: Verify Integration Management', () => {
    test('should retrieve user integrations', async () => {
      console.log('\n=== Step 3: Retrieving User Integrations ===');
      
      const integrations = await PipedreamUserIntegrations.getUserIntegrations(TEST_CONFIG.testUserId);
    ...
    test('should generate MCP configuration', async () => {
  ...
  describe('Step 4: Test App Details and Components', () => {
    ...
    test('should get app components (actions)', async () => {
      console.log('\n=== Step 4b: Getting App Components ===');
      
      const components = await PipedreamComponents.getAppComponents(TEST_CONFIG.testApp, 'actions');
  ...
  describe('Step 5: Test Available Integrations', () => {
    test('should get available integrations', async () => {
      ...
      const testAppIntegration = integrations.find(int => 
