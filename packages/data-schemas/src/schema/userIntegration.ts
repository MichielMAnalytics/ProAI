...
function hasIncrementalMCPSupport(): boolean {
  try {
    if (typeof require !== 'undefined') {
      ...
      return (
        typeof instance.connectSingleMCPServer === 'function' &&
...
async function smartMCPCacheInvalidation(userId: string, operation: string, doc?: any) {
  ...
  try {
    ...
    if (typeof require !== 'undefined') {
      ...
      if (
        hasIncremental &&
        (operation === 'save' || operation === 'update' || operation === 'findOneAndUpdate')
      ) {
        ...
        logger.info(
          `[UserIntegration] 🎯 INCREMENTAL mode: Skipping automatic cache clear for ${operation} - incremental operations will handle cache updates for user ${userId}`,
        );
        ...
        if (doc && doc.isActive === false) {
          logger.info(
            `[UserIntegration] 📝 Integration deactivated for user ${userId}, tools will be cleaned up when agents are next modified`,
          );
        }
      ...
      if (hasIncremental && operation === 'delete' && doc && doc.mcpServerConfig?.serverName) {
        logger.info(
          `[UserIntegration] 🎯 INCREMENTAL DELETE mode: Attempting incremental disconnect for server '${doc.mcpServerConfig.serverName}' for user ${userId}`,
        );
        ...
        try {
          ...
          if (result.success) {
            logger.info(
              `[UserIntegration] ✅ INCREMENTAL DELETE successful: Disconnected server '${doc.mcpServerConfig.serverName}' for user ${userId}`,
            );

            return; // Skip full cache clearing
          } else {
            logger.warn(
              `[UserIntegration] ⚠️ INCREMENTAL DELETE failed: ${result.error}. Falling back to full cache clear.`,
            );
          }
          ...
          logger.warn(
            `[UserIntegration] ⚠️ INCREMENTAL DELETE error: ${errorMessage}. Falling back to full cache clear.`,
          );
        }
      }
      ...
      logger.info(
        `[UserIntegration] 🔄 LEGACY mode: Performing full cache clear for ${operation} for user ${userId} (incremental available: ${hasIncremental})`,
      );
      ...
      logger.info(
        `[UserIntegration] ✅ CLEARED MCP cache via MCPInitializer for user ${userId} after ${operation}`,
      );
    }
  } catch (error: unknown) {
    ...
    if (typeof console !== 'undefined') {
      ...
      console.debug(
        `[UserIntegration] Could not perform smart cache invalidation: ${errorMessage}`,
      );
    }
  }
}
