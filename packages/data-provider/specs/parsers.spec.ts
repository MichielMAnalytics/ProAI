...
describe('replaceSpecialVars', () => {
  ...
  test('should remove {{mcp_servers}} placeholder as it is no longer supported', () => {
    const result = replaceSpecialVars({
      text: 'Available MCP servers: {{mcp_servers}}',
    });
    expect(result).toBe('Available MCP servers: ');
  });
  ...
  test('should confirm all specialVariables from config.ts get parsed', () => {
    ...
    // Note: mcp_servers placeholder is now removed from text, so no content expected
