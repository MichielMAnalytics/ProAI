...
export const useWorkflowNotifications = (options: UseWorkflowNotificationsOptions = {}) => {
  ...
  const [notificationQueue, setNotificationQueue] = useState<Map<string, QueuedNotification[]>>(new Map());
  ...
  const processNotification = useCallback((data: any) => {
    ...
    switch (data.notificationType) {
      ...
      case 'execution_failed':
        console.log('[WorkflowNotifications] Workflow execution complete:', data.notificationType);
        ...
        options.onTestComplete?.(workflowId, data.notificationType === 'execution_completed', executionResult);
        break;
    }
  }, [extractStepData, options]);

  // Process notification queue for a workflow
  const processNotificationQueue = useCallback((workflowId: string) => {
    setNotificationQueue(prev => {
      ...
      if (nextNotification.data.notificationType === 'step_completed' || 
          nextNotification.data.notificationType === 'step_failed') {
        console.log('[WorkflowNotifications] Delaying next notification processing for 2.5 seconds');
        setTimeout(() => {
  ...
  const handleWorkflowNotification = useCallback((event: CustomEvent) => {
    ...
    if (data.type === 'workflow_status_update' && data.workflowId) {
      ...
      if (options.workflowId && workflowId !== options.workflowId) {
        console.log('[WorkflowNotifications] Ignoring notification for different workflow:', workflowId);
        return;
      }
  ...
  useEffect(() => {
    console.log('[WorkflowNotifications] Setting up event listener for workflow:', options.workflowId);
    ...
    return () => {
      console.log('[WorkflowNotifications] Cleaning up event listener for workflow:', options.workflowId);
      window.removeEventListener('workflowNotification', handleWorkflowNotification as EventListener);
    };
